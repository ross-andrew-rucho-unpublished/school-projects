/*
Ross Rucho
10/30/2019
COMP 530 HW5 530client.c

A client program that uses the service of a distributed shell

This program reads input strings from stdin and sends them to
a server which executes the commands represented by the strings
and returns the string that represents the response of the command
back to the client.  The client then prints the response string to stdout.

The client has two positional parameters: (1) the DNS host name
where the server program is running, and (2) the port number of
the server's "welcoming" socket.

Honor Pledge

On my honor, I pledge that I have neither given nor received aid on this assignment

Ross Rucho
*/

#include <stdlib.h>
#include <stdio.h>
#include <stdarg.h>
#include <string.h>

#include "Socket.h"

#define MAX_LINE 10000 // Arbitrary limit of 10000 characters

int main(int argc, char* argv[])
{
  int i, c, rc;
  int count = 0;

  char line_data[MAX_LINE]; 

  /* variable to hold socket descriptor */
  Socket connect_socket;

  if (argc < 3)
     {
      printf("No host and port\n");
      return (-1);
     }

  /* connect to the server at the specified host and port;
   * blocks the process until the connection is accepted
   * by the server; creates a new data transfer socket.
   */
  connect_socket = Socket_new(argv[1], atoi(argv[2]));
  if (connect_socket < 0)
      {
       printf("Failed to connect to server\n");
       return (-1);
      }

  /* get a string from stdin up to and including
   * a newline or to the maximum input line size.
   * Continue getting strings from stdin until EOF.
   */

  // Start every command prompt line with the string "% "
  printf("%% ");

  while ((fgets(line_data, sizeof(line_data), stdin) != NULL))
  {
    count = strlen(line_data);  // Count does not include the '\0'

    /* send the characters of the input line to the server
     * using the data transfer socket.
     */
    for (i = 0; i < count; i++)
    {
      c = line_data[i];
      rc = Socket_putc(c, connect_socket);
      if (rc == EOF)
      {
        printf("Socket_putc EOF or error\n");
        Socket_close(connect_socket);
        exit (-1);  /* assume socket problem is fatal, end client */
      }
    }

    /* receive the converted characters for the string from
     * the server using the data transfer socket.  Expect the
     * server to return exactly the same number of characters
     * sent to it.
     */

    // Define local variables
    char currentChar;
    int ind=0;

    // Continuously read single characters from the socket
    do {
      // Acquire current character
      currentChar = Socket_getc(connect_socket);

      // Variable execution for acquired character
      if(currentChar==EOF) {
        printf("Socket_getc EOF or error\n");
        Socket_close(connect_socket);
        exit(-1);  // Assume socket problem is fatal. End client
      }else {
        line_data[ind] = currentChar;
        if(line_data[ind]=='\0') {
          break;
        }
      }

      // Increment the index
      ind++;
    }while(currentChar!='\0');

    // display the converted string on stdout
    printf("%s", line_data);

    // Start every command prompt line with the string "% "
    printf("%% ");

   } /* end of while loop; at EOF */

  Socket_close(connect_socket);
  exit(0);
}
